...
contract Verifier {
    ...
    uint256 constant ALPHA_X =
        17089284114625238894102314074898653082153325840917735085405711811526345768459;
    uint256 constant ALPHA_Y =
        15459607243514648577378299275490105854130190923724494670332431497453887679366;

    // Groth16 beta point in G2 in powers of i
    uint256 constant BETA_NEG_X_0 =
        4690900069922844500409170029104515442310837399869112087027104408443993448876;
    uint256 constant BETA_NEG_X_1 =
        14727886733740311103172842382766859085860481630127912186365596227382860586330;
    uint256 constant BETA_NEG_Y_0 =
        6074574397732650811379198425381900866389454931385154422234851555978084527409;
    uint256 constant BETA_NEG_Y_1 =
        11915647838402130673650820424619825807482484040943049158912351782430160086093;

    // Groth16 gamma point in G2 in powers of i
    uint256 constant GAMMA_NEG_X_0 =
        16841523895272257128105086960493488445185382450063608266293899015391874377695;
    uint256 constant GAMMA_NEG_X_1 =
        1722033021829299376534541172703782559796692999626331988804386477147224096171;
    uint256 constant GAMMA_NEG_Y_0 =
        11912021453598804778826320927201955617404449461204482368118535822183018259967;
    uint256 constant GAMMA_NEG_Y_1 =
        11320360068850620500713653285678793361241773032807892602345750098315331995441;

    // Groth16 delta point in G2 in powers of i
    uint256 constant DELTA_NEG_X_0 =
        1568413317742000514249200149955324231698971509771046921762170134220881735988;
    uint256 constant DELTA_NEG_X_1 =
        21846241470863179817766916748586102515513335910173645339915651555868249861813;
    uint256 constant DELTA_NEG_Y_0 =
        2867625869584959002492595092948584406405095398600412533528790367953733127853;
    uint256 constant DELTA_NEG_Y_1 =
        956695886599639068636913208428994313084349154448593013875881138994936583326;
    // Pedersen G point in G2 in powers of i
    uint256 constant PEDERSEN_G_X_0 =
        1814427085611285680918671750011527485743312529650469984378364316652805578938;
    uint256 constant PEDERSEN_G_X_1 =
        2873714209342015269874598482757036369209863347404354045898450718464941795628;
    uint256 constant PEDERSEN_G_Y_0 =
        18643199563098367401452096349492608268944355091535341537056735345161080409348;
    uint256 constant PEDERSEN_G_Y_1 =
        17182245194342064577533329972725030150684802170222914268971610815847895527990;

    // Pedersen GRootSigmaNeg point in G2 in powers of i
    uint256 constant PEDERSEN_GROOTSIGMANEG_X_0 =
        4092657951720167667901532041615134484718156565021746128564058715079261923199;
    uint256 constant PEDERSEN_GROOTSIGMANEG_X_1 =
        18905841583725949319348591670337944585868694729609423711143656488788291993761;
    uint256 constant PEDERSEN_GROOTSIGMANEG_Y_0 =
        14363382762252549935611739590282944000937525866877548960362815139873804853284;
    uint256 constant PEDERSEN_GROOTSIGMANEG_Y_1 =
        8042630018354888338924923848812792471162879518818942335045227240791639178229;

    // Constant and public input points
    uint256 constant CONSTANT_X =
        12978457168321044452806491611791154085985354453479526481835339722159507698175;
    uint256 constant CONSTANT_Y =
        16002853374256481637940778791471228951851773919179874702815444294174139579126;
    uint256 constant PUB_0_X =
        12445954358087182050022812773843052089004872124469431921765483985307708644283;
    uint256 constant PUB_0_Y =
        12885644010825123639120660412594308616239423793520876944218550381745808979466;
    uint256 constant PUB_1_X =
        21131588845405918271617998420676878811566197812838124179910857873393742405667;
    uint256 constant PUB_1_Y =
        13004887029492050439819964635680521313648509416568198172482274633055595552931;
    uint256 constant PUB_2_X =
        20249840985687068074067730564444025312224368318038379262909901666998698306901;
    uint256 constant PUB_2_Y =
        21633400757778632393787570872485277717763317370905000031979146532744659940962;
    ...
    function publicInputMSM(
        ...
        assembly ("memory-safe") {
            ...
            success := and(
                success,
                staticcall(gas(), PRECOMPILE_ADD, commitments, 64, g, 0x40)
            )
            success := and(
                success,
        ...
        )
    {
        ...
        compressedCommitmentPok = compress_g1(
            commitmentPok[0],
            commitmentPok[1]
        );
    }
    ...
    function verifyCompressedProof(
        ...
        {
            (commitments[0], commitments[1]) = decompress_g1(
                compressedCommitments[0]
            );
