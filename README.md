...
```bash
FOUNDRY_PROFILE=deploy forge script ./script/deploy/v2.0.0/SP1VerifierPlonk.s.sol:SP1VerifierScript --private-key $PRIVATE_KEY --verify --verifier etherscan --multi --broadcast
```

Change `v2.0.0` to the desired version to add.
...
```bash
FOUNDRY_PROFILE=deploy forge script ./script/deploy/v2.0.0/SP1VerifierPlonk.s.sol:SP1VerifierScript --private-key $PRIVATE_KEY --verify --verifier etherscan --multi --broadcast --sig "freeze()"
```

Change `v2.0.0` to the desired version to freeze.
